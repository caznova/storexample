...
It performs:
    ...
    3.1 Turn on AE, reset the controller if AE is already set
        ...
        "Indicate that system software is AHCI aware by setting GHC.AE to ??"
...
--*/
{
    ...
    if (pxis.PRCS) {
        ...
        //Note that PxSERR.DIAG.N is also set to ??on insertions and during interface power management entry/exit.
        serrMask.DIAG.N = 1;
        StorPortWriteRegisterUlong(ChannelExtension->AdapterExtension, &ChannelExtension->Px->SERR.AsUlong, serrMask.AsUlong);
