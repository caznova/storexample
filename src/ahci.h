...
typedef union _AHCI_INTERRUPT_STATUS {
    ...
    struct {
        ...
        ULONG DMPS :1; //Device Mechanical Presence Status (DMPS): When set, indicates that a mechanical presence switch attached to this port has been opened or closed, which may lead to a change in the connection state of the device.  This bit is only valid if both CAP.SMPS and P0CMD.MPSP are set to ??
...
typedef union _AHCI_INTERRUPT_ENABLE {
    ...
    struct {
        ...
        ULONG DMPE :1; //Device Mechanical Presence Enable (DMPE): When set, and GHC.IE is set to ?? and P0IS.DMPS is set, the HBA shall generate an interrupt.  For systems that do not support a mechanical presence switch, this bit shall be a read-only ??
...
typedef union _AHCI_COMMAND {
    ...
    struct {
        ...
        ULONG SUD :1; // Spin-Up Device: This bit is read/write for HBAs that support staggered spin-up via CAP.SSS. This bit is read only ?1 for HBAs that do not support staggered spin-up. On an edge detect from ?0 to ?1 , the HBA shall start a COMRESET initialization sequence to the device. Clearing this bit to ??does not cause any OOB signal to be sent on the interface.  When this bit is cleared to ??and PxSCTL.DET=0h, the HBA will enter listen mode as detailed in section 10.9.1.
...
typedef union _AHCI_DEVICE_SLEEP {
    ...
    struct {
        //LSB
        ULONG ADSE  :1;     // Aggressive Device Sleep Enable (ADSE): When this bit is set to ?? the HBA shall assert the DEVSLP signal after the port has been idle (PxCI = 0h and PxSACT = 0h) for the amount of time specified by the PxDEVSLP.DITO register.  When this bit is set to ??and CAP2.DESO is set to ?? the HBA shall assert the DEVSLP signal after the port has been idle (PxCI = 0h and PxSACT = 0h) for the amount of time specified by PxDEVSLP.DITO and the interface is in Slumber (PxSSTS.IPM = 6h).When this bit is cleared to ?? the HBA does not enter DevSleep unless software directed via PxCMD.ICC.  This bit shall only be set to ??if PxDEVSLP.DSP is set to ??
        ULONG DSP   :1;     // Device Sleep Present (DSP): If set to ?? the platform supports Device Sleep on this port. If cleared to ?? the platform does not support Device Sleep on this port. This bit may only be set to ??if CAP2.SDS is set to ?? DSP is mutually exclusive with the PxCMD.HPCP bit and PxCMD.ESP bit.
...
typedef union _AHCI_HBA_CAPABILITIES  {
    ...
    struct {
        ...
        ULONG SXS :1; //AHCI 1.1 Supports External SATA: When set to ?? indicates that the HBA has one or more Serial ATA ports that has a signal only connector that is externally accessible. If this bit is set, software may refer to the PxCMD.ESP bit to determine whether a specific port has its signal connector externally accessible as a signal only connector (i.e. power is not part of that connector).  When the bit is cleared to ?? indicates that the HBA has no Serial ATA ports that have a signal only connector externally accessible.
        ULONG EMS :1; //AHCI 1.1 Enclosure Management Supported: When set to ?? indicates that the HBA supports enclosure management as defined in section 12.  When enclosure management is supported, the HBA has implemented the EM_LOC and EM_CTL global HBA registers.  When cleared to ?? indicates that the HBA does not support enclosure management and the EM_LOC and EM_CTL global HBA registers are not implemented.
        ULONG CCCS :1; //AHCI 1.1 Command Completion Coalescing Supported: When set to ?? indicates that the HBA supports command completion coalescing as defined in section 11.  When command completion coalescing is supported, the HBA has implemented the CCC_CTL and the CCC_PORTS global HBA registers.  When cleared to ?? indicates that the HBA does not support command completion coalescing and the CCC_CTL and CCC_PORTS global HBA registers are not implemented.
        ...
        ULONG PMD :1; //PIO Multiple DRQ Block: If set to ? , the HBA supports multiple DRQ block data transfers for the PIO command protocol. If cleared to ?0 the HBA only supports single DRQ block data transfers for the PIO command protocol.
        ULONG FBSS :1; //AHCI 1.1  FIS-based Switching Supported: When set to ?? indicates that the HBA supports Port Multiplier FIS-based switching. When cleared to ?? indicates that the HBA does not support FIS-based switching.  AHCI 1.0 and 1.1 HBAs shall have this bit cleared to ??
        ...
        ULONG SNZO :1; //Supports Non-Zero DMA Offsets: When set to ? , indicates that the HBA can support non-zero DMA offsets for DMA Setup FISes. This bit is reserved for future AHCI enhancements. AHCI 1.0 HBAs must have this bit cleared to ?0 .
        ...
        ULONG SCLO :1; //Supports Command List Override: When set to ? , indicates that the HBA supports the PxCMD.CLO bit and its associated function. When cleared to ?0 , the HBA is not capable of clearing the BSY and DRQ bits in the Status register in order to issue a software reset if these bits are still set from a previous operation.
        ULONG SAL :1; //Supports Activity LED: When set to ? , indicates that the HBA supports a single output pin which indicates activity. This pin can be connected to an LED on the platform to indicate device activity on any drive. See section 10.10 for more information.
        ...
        ULONG SSS :1; //Supports Staggered Spin-up: When set to ? , indicates that the HBA supports staggered spin-up on its ports, for use in balancing power spikes. This value is loaded by the BIOS prior to OS initialization.
        ULONG SMPS :1; //Supports Mechanical Presence Switch: ): When set to ?? the HBA supports mechanical presence switches on its ports for use in hot plug operations.  When cleared to ?? this function is not supported.  This value is loaded by the BIOS prior to OS initialization.
        ULONG SSNTF :1; //AHCI 1.1 Supports SNotification Register: When set to ?? indicates that the HBA supports the PxSNTF (SNotification) register and its associated functionality.  When cleared to ?? the HBA does not support the PxSNTF (SNotification) register and its associated functionality.  Refer to section 10.10.1.
...
typedef union _AHCI_Global_HBA_CONTROL {
    ...
    struct {
        ...
        ULONG MRSM :1; //MSI Revert to Single Message: When set to ??by hardware, indicates that the HBA requested more than one MSI vector but has reverted to using the first vector only.  When this bit is cleared to ?? the HBA has not reverted to single MSI mode (i.e. hardware is already in single MSI mode, software has allocated the number of messages requested, or hardware is sharing interrupt vectors if MC.MME < MC.MMC).
...
typedef union _AHCI_COMMAND_COMPLETION_COALESCING_CONTROL {
        ...
        struct {
        //LSB
        ULONG EN: 1; //Enable: When cleared to ?? the command completion coalescing feature is disabled and no CCC interrupts are generated.  When set to ?? the command completion coalescing feature is enabled and CCC interrupts may be generated based on timeout or command completion conditions.  Software shall only change the contents of the TV and CC fields when EN is cleared to ?? On transition of this bit from ??to ?? any updated values for the TV and CC fields shall take effect.
        ULONG Reserved: 2;
        ULONG INT: 5; //Interrupt: Specifies the interrupt used by the CCC feature.  This interrupt must be marked as unused in the Ports Implemented (PI) register by the corresponding bit being set to ??  Thus, the CCC interrupt corresponds to the interrupt for an unimplemented port on the controller.  When a CCC interrupt occurs, the IS.IPS[INT] bit shall be asserted to ??  This field also specifies the interrupt vector used for MSI.
        ULONG CC: 8; //Command Completions: Specifies the number of command completions that are necessary to cause a CCC interrupt.  The HBA has an internal command completion counter, hCccComplete.  hCccComplete is incremented by one each time a selected port has a command completion.  When hCccComplete is equal to the command completions value, a CCC interrupt is signaled.  The internal command completion counter is reset to ??on the assertion of each CCC interrupt.  A value of ??for this field shall disable CCC interrupts being generated based on the number of commands completed, i.e. CCC interrupts are only generated based on the timer in this case.
        ULONG TV: 16; //Timeout Value: The timeout value is specified in 1 millisecond intervals.  The timer accuracy shall be within 5%.  hCccTimer is loaded with this timeout value.  hCccTimer is only decremented when commands are outstanding on selected ports, as defined in section 11.2.  The HBA will signal a CCC interrupt when hCccTimer has decremented to ??  hCccTimer is reset to the timeout value on the assertion of each CCC interrupt.  A timeout value of ?? is reserved.
        //MSB
    };
...
typedef union _AHCI_ENCLOSURE_MANAGEMENT_LOCATION {
    ...
    struct {
        //LSB
        ULONG SZ :16; //Buffer Size: Specifies the size of the transmit message buffer area in Dwords.  If both transmit and receive buffers are supported, then the transmit buffer begins at ABAR[EM_LOC.OFST*4] and the receive buffer directly follows it.  If both transmit and receive buffers are supported, both buffers are of the size indicated in the Buffer Size field.  A value of ??is invalid.
...
typedef union _AHCI_ENCLOSURE_MANAGEMENT_CONTROL {
    ...
    struct {
        //LSB
        ULONG STS_MR :1; //Message Received: The HBA sets this bit to a ??when a message is completely received into the message buffer.  When software detects this bit is a ?? software should read the message and perform any actions necessary.  When software is finished reading the message in the buffer, software writes a ??to this bit in order to clear it.  A write of ??to this bit by software shall have no effect.
        ULONG Reserved0 :7;
        ULONG CTL_TM :1; //Transmit Message: When set to ??by software, the HBA shall transmit the message contained in the message buffer.  When the message is completely sent, the HBA shall clear this bit to ??  A write of ??to this bit by software shall have no effect.  Software shall not change the contents of the message buffer while CTL.TM is set to ??
        ULONG CTL_RST :1; //Reset: When set to ??by software, the HBA shall reset all enclosure management message logic and the attached enclosure processor (if applicable) and take all appropriate reset actions to ensure messages can be transmitted/received after the reset.  After the HBA completes the reset operation, the HBA shall set the value to ??  A write of ??by software to this field shall have no effect.
        ULONG Reserved1 :6;
        ULONG SUPP_LED :1; //LED Message Types:  If set to ?? the HBA supports the LED message type defined in section 12.2.1.
        ULONG SUPP_SAFTE :1; //SAF-TE Enclosure Management Messages:  If set to ?? the HBA supports the SAF-TE message type.
        ULONG SUPP_SES2 :1; //SES-2 Enclosure Management Messages:  If set to ?? the HBA supports the SES-2 message type.
        ULONG SUPP_SGPIO :1; //SGPIO Enclosure Management Messages:  If set to ?? the HBA supports the SGPIO register interface message type.
        ULONG Reserved2 :4;
        ULONG ATTR_SMB :1; //Single Message Buffer:  If set to ?? the HBA has one message buffer that is shared for messages to transmit and messages received.  In this case, unsolicited receive messages are not supported and it is software¡¯s responsibility to manage access to this buffer.  If cleared to ?? there are separate receive and transmit buffers such that unsolicited messages could be supported.
        ULONG ATTR_XMT :1; //Transmit Only:  If set to ?? the HBA only supports transmitting messages and does not support receiving messages.  If cleared to ?? the HBA supports transmitting and receiving messages.
        ULONG ATTR_ALHD :1; //Activity LED Hardware Driven:  If set to ?? the HBA drives the activity LED for the LED message type in hardware and does not utilize software settings for this LED.  The HBA does not begin transmitting the hardware based activity signal until after software has written CTL.TM=1 after a reset condition.
        ULONG ATTR_PM :1; //Port Multiplier Support: If set to ?? the HBA supports enclosure management messages for devices attached via a Port Multiplier.  If cleared to ?? the HBA does not support enclosure management messages for devices attached via a Port Multiplier.  When cleared to ?? software should use the Serial ATA enclosure management bridge that is built into many Port Multipliers for enclosure services with these devices.  For more information on Serial ATA enclosure management bridges, refer to the Serial ATA Revision 2.5 specification.
        ULONG Reserved3 :4;
        //MSB
    };
...
typedef union _AHCI_HBA_CAPABILITIES2 {
    ...
    struct {
        //LSB
        ULONG BOH  :1; //RO -- BIOS/OS Handoff:  When set to ?? the HBA supports the BIOS/OS handoff mechanism defined in section TBD.  When cleared to ?? the HBA does not support the BIOS/OS handoff mechanism.
        ULONG NVMP :1; //RO -- NVMHCI Present: When set to '1', the HBA includes support for NVMHCI and the registers at offset 60h-9Fh are valid.  When cleared to ?? the HBA does not support NVMHCI.
        ULONG APST :1; //RO -- Automatic Partial to Slumber Transitions (APST): When set to ?? the HBA supports Automatic Partial to Slumber Transitions.  When cleared to ?? Automatic Partial to Slumber Transitions are not supported.
        ULONG SDS  :1; //RO -- Supports Device Sleep (SDS): When set to ?? the HBA supports the Device Sleep feature. When cleared to ?? DEVSLP is not supported and software shall not set PxCMD.ICC to ?h?
        ULONG SADM :1; //RO -- Supports Aggressive Device Sleep Management (SADM): When set to ?? the HBA supports hardware assertion of the DEVSLP signal after the idle timeout expires. When cleared to ?? this function is not supported and software shall treat the PxDEVSLP.ADSE field as reserved. Refer to section 8.5.1.
        ULONG DESO :1; //RO -- DevSleep Entrance from Slumber Only (DESO):  This field specifies that the HBA shall only assert the DEVSLP signal if the interface is in Slumber.  When this bit is set to ?? the HBA shall ignore software directed entrance to DevSleep via PxCMD.ICC unless PxSSTS.IPM = 6h. When this bit is cleared to ?? the HBA may enter DevSleep from any link state (active, Partial, or Slumber).
